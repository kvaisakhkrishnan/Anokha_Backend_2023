const createTables = (db) => {
    ...
    db.query("create table EventData (eventId int PRIMARY KEY AUTO_INCREMENT, eventName varchar(50) NOT NULL,eventOrWorkshop boolean NOT NULL, technical boolean not null, groupOrIndividual bool NOT NULL,minCount int not null,  maxCount int NOT NULL, description varchar(500) NOT NULL, url varchar(300) not null, userEmail varchar(65) not null,date varchar(20) NOT NULL,eventTime time NOT NULL,venue varchar(50) NOT NULL,fees int NOT NULL,totalNumberOfSeats int NOT NULL,noOfRegistrations int NOT NULL,timeStamp timestamp NOT NULL,refundable boolean NOT NULL,departmentAbbr varchar(10) not null,FOREIGN KEY(departmentAbbr) REFERENCES DepartmentData(departmentAbbr),FOREIGN KEY(userEmail) REFERENCES EventManager(userEmail))", (err,result) => {
        if(err) {
