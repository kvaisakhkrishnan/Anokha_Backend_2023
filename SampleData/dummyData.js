const insertDummyData = (db, transactions_db) => {
    ...
    for(var i = 0; i<=9; i+=1)
    {
        ...
        db.query(`insert into EventData (eventName, eventOrWorkshop, technical , groupOrIndividual, maxCount, description, url, userEmail, date, eventTime, venue, fees, totalNumberOfSeats, noOfRegistrations, timeStamp, refundable, departmentAbbr) values ('EVENT${i}', ${bool_data}, ${bool_data},${!bool_data}, 1, 'DESC${i}', 'https://play-lh.googleusercontent.com/VojafVZNddI6JvdDGWFrRmxc-prrcInL2AuBymsqGoeXjT4f9sv7KnetB-v3iLxk_Koi','managerEmail${i + 1}@gmail.com', '24-Mar-2023', '08:14:57', 'VENUE${i}', 100, 100, 0, '${date_time}', 0,  'DEP${i}')`)
