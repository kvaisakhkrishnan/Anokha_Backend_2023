const insertDummyData = (db, transactions_db) => {
    ...
    for(var i = 0; i<=9; i+=1)
    {
        ...
        db.query(`insert into UserData (userEmail, fullName, password, phoneNumber, currentStatus, activePassport, isAmritaCBE, collegeId, accountTimeStamp, passportId, passportTimeStamp) values ('cb.en.u4cse2001${i}@cb.students.amrita.edu', 'FIRSTNAME${i} LASTNAME${i}', 'SAFEPASSWORD${i}', '123456789${i}' ,${!bool_data}, ${bool_data}, ${bool_data}, ${i + 1},'${date_time}','PASSPORTID${i}', '${date_time}')`);
        ...
        db.query(`insert into EventData (eventName, eventOrWorkshop, technical , groupOrIndividual,minCount,  maxCount, description, url, userEmail, date, eventTime, venue, fees, totalNumberOfSeats, noOfRegistrations, timeStamp, refundable, departmentAbbr) values ('EVENT${i}', ${bool_data}, ${bool_data},${!bool_data}, 1, 1, 'DESC${i}', 'https://play-lh.googleusercontent.com/VojafVZNddI6JvdDGWFrRmxc-prrcInL2AuBymsqGoeXjT4f9sv7KnetB-v3iLxk_Koi','managerEmail${i + 1}@gmail.com', '24-Mar-2023', '08:14:57', 'VENUE${i}', 100, 100, 0, '${date_time}', 0,  'DEP${i}')`)
       ...
       transactions_db.query(`insert into Transactions (transactionId,productId, userEmail,senderName, eventIdOrPassportId,amount,timeStamp, transactionStatus, address ,city, state , country, zipcode, phoneNumber ) values ('ANK738278372873${i}', 'PID${i}', 'cb.en.u4cse2001${i}@cb.students.amrita.edu', 'SENDER${i}', 0, 100,'${date_time}', 'INITIATED', 'ADDRESS${i}', 'CITY${i}', 'STATE${i}','INDIA', '98765432${i}', '666634${i}')`);
